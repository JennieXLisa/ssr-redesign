...
is v0.16. P1-D84-P1-D89 add individually identified, non-overlapping search
occurrences, deterministic file/byte ordering, continuation after the complete
match rather than its preview, preserved original-file matching context, and
explicit failure on produced zero-length matches. An occurrence omitted by a
response limit must not be skipped silently.

R96-R101, section 6.13 (E1-E6), and T137-T144 capture this approval. Exact regex
engine/dialect/flags and competing-alternative semantics, encoding and case-to-byte
mapping, path comparator, search cursor/recovery and resource contracts remain
open. Earlier source, cursor, index, analysis-retrieval, freshness, relationship,
file-discovery and cross-line matching decisions remain intact. No future search
continuation design is implied by this save.

P1-F02 remains IN DISCUSSION and NOT READY. This amendment is local only; no
GitHub access/write or remote-head verification occurred. No application code,
dependency or runtime state changed, and no application tests ran. The incremental
patch uses exact local v0.15/v1.15 preimages preserved under
`history/before-p1-f02-occurrence-enumeration/`, not an assumed remote baseline.

## Explicitly unresolved — do not promote to requirements

| ID | Open choice | Owning phase |
|---|---|---|
| O-001 | Partially resolved by P1-D01–P1-D89 above: access, synthesis, return to research and tool directions are agreed. Exact feature contracts and role-to-code mapping remain open. | 1 |
