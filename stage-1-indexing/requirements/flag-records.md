...
Status: approved behavior from the researcher discussion. This records the agreed observation fields, display grouping, custom-rule support, rule-selection modes, conflicting-ID handling, identical-rule deduplication, pre-scan validation, and local-name-only wildcard/regex matching; it is not a complete database schema, rule catalogue, implementation plan, or test-results report.
...
**S1-FL-R11 — Wildcard and regex matching of local names only.** Support two explicitly selected interesting-name matching modes: wildcard and regular expression. Apply both modes to indexed local names only. Do not add a local/qualified-name selector or automatically match both fields. Qualified names remain available as record and display context under S1-FN-R02, not as name-rule matching inputs. Do not add a separate exact-match mode. Wildcard matching applies to the whole local name: an ordinary pattern without wildcard operators, such as `execute_command`, matches that name rather than names merely containing it; `*command*` matches local names containing that text. This provides ordinary literal-name matching without another mode.

Each name rule specifies its matching mode and case sensitivity. Do not infer the mode from punctuation or silently reinterpret an invalid pattern as another mode. Match indexed local names, not arbitrary comment or string contents; separate Semgrep code-pattern rules remain unaffected. Apply the existing pre-scan validation and frozen-rule provenance requirements to these settings. The regex engine/dialect, wildcard grammar and escaping, and regex search-versus-full-match behavior remain to be specified. This agreement does not select those mechanisms or introduce a custom pattern engine.
...
| S1-FL-T20 | Match selected names `execute_command`, `pre_execute_command`, and `execute_command_safe` with wildcard patterns `execute_command` and `*command*`. | The ordinary wildcard pattern matches only the first name; the wildcard-containing pattern matches all three. No separate exact-match mode is required. |
| S1-FL-T21 | Select wildcard and regex name rules with explicit case sensitivity, and include the same text in indexed names, comments, and strings. | Apply only the declared mode and case setting to the selected indexed names; do not flag comment/string mentions through name matching, infer a different mode from punctuation, or reinterpret an invalid pattern to obtain a match. Preserve rule settings in the run's frozen selection. |
| S1-FL-T22 | Apply case-sensitive wildcard `*command*` and regex `.*command.*` rules to functions `Admin::execute_command`, `Worker::execute_command`, and `command_manager::run`. | Both rules match the first two functions through their local name, preserving each symbol's identity and qualified-name context. Neither flags `run` merely because its enclosing scope contains `command`. No local/qualified-name selection is required or offered. |

## Decisions still requiring discussion

- The category catalogue, rule-input format, exact selection options, empty custom-only selection handling, and exact identical-definition comparison. Custom-rule support, selection modes, conflicting-ID rejection, and identical-rule deduplication are settled by S1-FL-R06/R07/R08/R09.
- Regex engine/dialect, wildcard grammar/escaping, and regex search-versus-full-match behavior. Wildcard/regex-only modes, explicit case sensitivity, and local-name-only matching are settled by S1-FL-R11; do not reintroduce a separate exact-match mode or local/qualified-name selector.
