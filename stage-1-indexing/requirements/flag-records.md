...
Status: approved behavior from the researcher discussion. This records the agreed observation fields, display grouping, custom-rule support, rule-selection modes, and conflicting-ID handling; it is not a complete database schema, rule catalogue, implementation plan, or test-results report.
...
- The category catalogue, rule-input format, exact selection options, empty custom-only selection handling, identical-definition comparison/deduplication, and name-matching semantics. Custom-rule support, selection modes, and conflicting-ID rejection are settled by S1-FL-R06/R07/R08.
