...
Status: approved behavior from the researcher discussion. This records the agreed observation fields, display grouping, custom-rule support, rule-selection modes, conflicting-ID handling, identical-rule deduplication, pre-scan validation, and wildcard/regex name matching; it is not a complete database schema, rule catalogue, implementation plan, or test-results report.
...
**S1-FL-R11 — Wildcard and regex name matching only.** Support two explicitly selected interesting-name matching modes: wildcard and regular expression. Do not add a separate exact-match mode. Wildcard matching applies to the whole selected name: an ordinary pattern without wildcard operators, such as `execute_command`, matches that name rather than names merely containing it; `*command*` matches names containing that text. This provides ordinary literal-name matching without another mode.

Each name rule specifies its matching mode and case sensitivity. Do not infer the mode from punctuation or silently reinterpret an invalid pattern as another mode. Match indexed names, not arbitrary comment or string contents; separate Semgrep code-pattern rules remain unaffected. Apply the existing pre-scan validation and frozen-rule provenance requirements to these settings. The regex engine/dialect, wildcard grammar and escaping, regex search-versus-full-match behavior, and selection of local versus qualified names remain to be specified. This agreement does not select those mechanisms or introduce a custom pattern engine.
...
| S1-FL-T05 | Query available flags while another required flagging component is unfinished. | Return bounded committed observations with explicit completeness information; distinguish incomplete empty results from completed applicable processing with no matches. |
...
- Regex engine/dialect, wildcard grammar/escaping, regex search-versus-full-match behavior, and local/qualified-name selection. Wildcard/regex-only modes and explicit case sensitivity are settled by S1-FL-R11; do not reintroduce a separate exact-match mode.
