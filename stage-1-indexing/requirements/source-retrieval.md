...
Status: approved behavior from the researcher discussion. This records the agreed lookup, reading, captured-text search, one-hop caller/callee navigation, file-outline, file-discovery, direct function-read reference, and self-contained repeatable-continuation requirements; it is not a complete executable API schema, implementation plan, or test-results report.
...
Use self-contained tokens under S1-NV-R14 rather than stored cursor sessions. Exact token encoding, lifetime/error behavior, and response-size policy remain to be specified with the researcher. This agreement does not settle pagination over changing search results.
...
This selects the continuation architecture only. Exact payload fields, encoding, integrity validation, byte-range conventions, lifetime/error behavior, and response budgeting remain to be specified with the researcher. It does not introduce a signing/key-management design, a cursor-session store, or a pagination design for changing search results.
...
- Large-source response budgets, exact continuation payload/encoding, integrity validation, and lifetime/error behavior. Repeatability and reader independence are settled by S1-NV-R13; self-contained tokens without cursor-session storage are settled by S1-NV-R14. S1-NV-R12 requires explicit occurrence references; declaration-reference representation and pagination for multiple occurrences remain to be specified.
