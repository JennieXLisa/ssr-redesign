...
This selects the continuation architecture only. Exact payload fields, encoding, integrity validation, byte-range conventions, lifetime/error behavior, and detailed byte accounting under S1-NV-R15 remain to be specified with the researcher. It does not introduce a signing/key-management design, a cursor-session store, or a pagination design for changing search results.
