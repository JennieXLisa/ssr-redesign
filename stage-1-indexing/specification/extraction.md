...
Each adapter implements `extract(source: SourceUnit) -> ExtractionBundle`, using the exact typed values in [records.py](../contracts/v1/records.py), not an adapter-local interpretation of this inventory. SourceUnit contains file/snapshot/extraction identity, original bytes, selected language, exact decoding/offset map and parser-profile digest. The bundle contains symbols, source occurrences, scopes, imports, lexical declarations/invalidation facts, reference/call occurrences, ordered arguments and diagnostics. It performs no SQL, subprocess build, network I/O or publication. Parser instances are local to a worker process; do not share mutable parser state between threads. W02 adopts these shared records; W07 implements their first producer rather than inventing their shape.
...
## Shared traversal and publication recipe

Build SourceUnit once from the frozen metadata: strip only the recognized BOM in the parser representation, preserve all newline bytes semantically, and record every decoded scalar's UTF-8/original boundary. SourceUnit validation rejects newline normalization, a wrong BOM, partial characters and nonmonotone/incomplete maps. Parser offsets can only be translated through this map; source-selection rules still treat CRLF as an indivisible paging atom.

Pass 1 allocates the module scope and syntax-owned scopes/symbols/occurrences in original-source order. Pass 2 emits parameters, declarations and assignment/deletion invalidations, imports/exports and call/non-call references using the evaluation scope chosen by the language policy. A callable's declaration belongs to its containing scope; its body gets a new callable scope. Defaults/decorators/capture initializers are visited in their language-defined evaluation scope before entering the body; unknown ownership is an explicit limitation, not an outer-function containment guess. Parameter symbols need not be fabricated, but their shadowing DeclarationRecord is mandatory. A CALL argument that names a callable can additionally produce CALLBACK_ARGUMENT; never a second fabricated CALL.

Every successful bundle has one module root scope, even for a valid zero-callable file. A failed parse emits FAILED plus diagnostics and empty record collections; it cannot publish a partially successful inventory. TypeFact and PropertyFact always retain declared/inferred/unknown origin, with null for unknown and original source evidence for known values. Missing modifiers are not automatically stored as inferred false. Constructor/destructor return types remain unknown unless the language specifies a written type. SQL projection follows storage.md; no adapter writes arbitrary properties/import JSON.

Before staging, validate all record types, unique identities, parent acyclicity, local foreign references, argument/parameter order and file bounds. Before activation, additionally check original source excerpts/line coordinates, selected profile identities, deterministic ID construction and parser success. The machine-readable golden fixture inputs and language-specific decision table are described in [language-policies.md](language-policies.md); self-consistency of authored fixtures is not evidence that the pinned grammar queries pass.
