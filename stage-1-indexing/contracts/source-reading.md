...
Status: operation signatures, the SourcePage fields, whole-response byte budgeting, the 50 KB default, and insufficient-budget handling below are approved. Permitted budget bounds, detailed range fields, token encoding, and other remaining items are not yet frozen. This is a partial API contract, not implemented functionality or an executed test report.
...
| `max_bytes` | Optional initial whole-response byte budget under S1-NV-R21. `None` uses the configured source-response budget, whose shipped default is **50,000 bytes (50 KB)** under S1-NV-R22. Insufficient budgets follow S1-NV-R23. Permitted bounds and the exact serialization profile remain to be agreed. |
...
Issued continuations retain their fixed section and effective budget under S1-NV-R13/R14/R20; whole-response accounting does not authorize resizing an already-issued section. The default is specified in S1-NV-R22 and insufficient-budget handling in S1-NV-R23. The concrete serialization/encoding profile, framing boundary for each adapter, permitted budget bounds, overhead reservation for changing coverage metadata, and the independent error-response size policy remain to be specified before implementation. Both the byte bound and repeatable source-section requirements must be preserved by that design.
...
Keep the default configurable and allow an explicit valid `max_bytes` on the initial read to override it. The 50 KB value is a default, not a hard maximum or a minimum. Permitted override bounds remain to be specified; insufficient budgets follow S1-NV-R23. Large functions remain fully available through continuation; the default must not become a capture, indexing, or total-function-size limit.
...
## Insufficient source-response budgets

**S1-NV-R23 — Reject budgets that cannot support a source page.** For nonempty selected source, when the effective initial budget cannot fit the required serialized response metadata, at least one complete source character, and a continuation when more source remains, return `RESPONSE_BUDGET_TOO_SMALL` in the shared `errors` array. This is budget validation, not an alternative to pagination. Source larger than a viable page budget uses continuation under the existing rules; the budget need not fit an entire line or function. Do not return an empty successful page, silently increase the requested size, drop required fields, or issue a continuation that makes no source progress to conceal an insufficient budget.

Identify the operation and affected `max_bytes` input or configured effective budget, explain what cannot fit, and report `requested_bytes` and `minimum_required_bytes`. Calculate the minimum from the required serialization, metadata, valid source fragment, and applicable continuation overhead; do not guess it or hard-code illustrative example sizes. Tell the caller to repeat the initial read with a sufficient `max_bytes`, preserving the selected run, symbol, and occurrence references. An unchanged insufficient request is not retryable. The error and its correction do not recapture source, discard indexing progress, or change an already-issued continuation's fixed section.

Error responses use their own bounded size policy, independent of the source-response `max_bytes`. An impossibly small source budget must not prevent an actionable error from being returned. This does not authorize unbounded diagnostics: the shared error-detail and sensitive-data rules still apply. The numeric error-response bound, complete error-item schema, exact minimum-calculation algorithm under the final serialization profile, and continuation-overhead reservation remain to be specified. Preserve both whole-response budgeting and fixed-section repeatability when defining those mechanisms.
...
3. For an initial read, resolve the default or explicit budget under S1-NV-R22 and apply whole-response accounting under S1-NV-R21, allowing for required metadata and serialization while retaining whole lines where possible. Reject an insufficient budget under S1-NV-R23 rather than returning a non-progressing page. Split an oversized line only at valid character boundaries and report the fragment's position. For continuation, honor its already-fixed section and validate it against the canonical records.
...
| S1-NV-T53 | Through the shared reader, request nonempty source with a budget below its calculated minimum, including an impossibly small budget that cannot contain an error response. | Return an actionable errors[] response identifying the operation and budget, the requested bytes, the calculated minimum required bytes, and how to repeat the initial read with sufficient size and the same source references. Do not return an empty successful page, silently increase the size, or issue a non-progressing continuation. The diagnostic follows its own bounded policy rather than being truncated to the invalid source budget; source and completed index records remain unchanged. |
| S1-NV-T54 | Use long metadata, escaped source text, and an oversized line with multibyte characters; compare a budget below the calculated minimum with a sufficient budget smaller than the whole line/function. | Minimum accounting includes required serialization and continuation overhead, not guessed constants or a requirement to fit the whole line. The insufficient request receives correction guidance. A viable budget returns a character-safe source fragment with continuation when needed, distinguishing ordinary pagination from invalid-budget handling. |

## Details still requiring agreement

- Permitted response-budget bounds; concrete serialization/encoding and adapter-framing boundaries under S1-NV-R21; overhead reservation for repeatable sections with changing metadata; the exact minimum-calculation algorithm and separately bounded error-response size under S1-NV-R23. The shipped default is fixed at 50,000 bytes by S1-NV-R22.
...
The operation shape, whole-response budgeting boundary, 50 KB default, and insufficient-budget behavior are settled. Do not add a continuation size override, stored cursor sessions, automatic source draining, or unrelated navigation features while filling in these remaining details.
